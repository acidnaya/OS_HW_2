...
void init(void) {
    ...
    if((clients = sem_open(clients_sem, O_CREAT, 0666, 1)) == 0) {
        perror("sem_open: Can not create clients semaphore");
        exit(-1);
    };
...
void close_all(void) {
    if(sem_close(clients) == -1) {
        perror("sem_close: Incorrect close of clients semaphore");
        exit(-1);
    };
}
...
void unlink_all(void) {
    ...
    if(sem_unlink(clients_sem) == -1) {
        perror("clients sem_unlink");
    }
